...
class MainEngine(object):
    ...
    def sendOrder(self, orderReq, gatewayName):
        ...
        str1 = 'symbol %s %s exchange %s price %.2f volume %d direction %s offset %s priceType %s'\
        %(orderReq.symbol, orderReq.vtSymbol, orderReq.exchange,orderReq.price,orderReq.volume,orderReq.direction,orderReq.offset,orderReq.priceType)
        str2 = 'IB相关  %s|%s|%s|%.2f|%s|%s|%s|'%(orderReq.productClass,orderReq.currency,orderReq.expiry,orderReq.strikePrice, \
        orderReq.optionType ,orderReq.lastTradeDateOrContractMonth ,orderReq.multiplier  )
        print str1 
    ...
    #----------------------------------------------------------------------
    def sendSignal(self, signal ):   #  g    VtSignal = type(signal) 
        #此处需要代码
        #self.sendMsg(u'信号', signal )
        pass
...
class DataEngine(object):
    ...
    def processTickEvent(self, event):
        ...
        # lastTick = self.tickDict[tick.vtSymbol]
        # if( tick.lastVolume == 0  ):
        #     tick.lastVolume = tick.volume - lastTick.volume
        self.tickDict[tick.vtSymbol] = tick   
