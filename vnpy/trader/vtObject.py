...
class VtTickData(VtBaseData):
    ...
    def __init__(self):
        ...
        self.avgPrice = EMPTY_FLOAT             # 均价          
        self.totalMoney = EMPTY_FLOAT           # 总交易额       
...
class VtOrderData(VtBaseData):
    ...
    def __str__(self):
        str = u'VtOrderData, vtSymbol %s vtOrderID %s status %s '%(self.vtSymbol, self.vtOrderID,  self.status)
        return str
...
class VtOrderReq(object):
    ...
    def __str__(self):
        """str"""
        str1 = 'symbol %s exchange %s price %.2f volume %d direction %s offset %s priceType %s'\
        %(self.symbol,self.exchange,self.price,self.volume,self.direction,self.offset,self.priceType)
        str2 = 'IB相关  %s|%s|%s|%.2f|%s|%s|%s|'%(self.productClass,self.currency,self.expiry,self.strikePrice, \
        self.optionType ,self.lastTradeDateOrContractMonth ,self.multiplier  )
        return  str1 # u'%s %s'%(str,str2)
