...
class CaodanAlgo(AlgoTemplate):
    ...
    def onPotDataGen(self, newPotData):
        ...
        msg = u'出方向 %d %.1f '%( self.type , self.potPrice )
        #print msg
        self.writeLog(msg)  
    ...
    def onTick(self, tick):
        ...
        if( self.goodlook and  self.type == POT_TYPE_UP and self.tickcount == 0  ):
            msg = u'%s Caodan入场，代码：%s， 方向:  %d 点价:%.1f 现价:%.1f' %(tick.time, self.vtSymbol,  self.type ,self.potPrice, tick.lastPrice  )
            self.writeLog(msg)  
            #print msg 
        if( self.goodlook and self.type == POT_TYPE_DOWN and self.tickcount == 0  ):
            msg = u'%s Caodan入场，代码：%s， 方向:  %d 点价:%.1f 现价:%.1f' %(tick.time, self.vtSymbol,  self.type ,self.potPrice, tick.lastPrice  )
            self.writeLog(msg)  
            #print msg 
        ...
        if( self.tickcount == 120 and self.goodlook == True ):
        ...
        if( income >= 3 and self.goodlook == True and self.tickcount>=90 ):
