...
'vtSymbol': 'rb1910'}        
        ...
        # txt = u'期货成交：' +  trade['orderID'] +  trade['direction'] +  trade['offset'] +  str(trade['Volume']) +  str(trade['price'])
        # sendwxmsg( txt )
    ...
    def processTimerEvent(self, event):
        ...
        fTime = getFutureTime()
        #print fTime  
        if( fTime['timeclass'] == 7  ):
            timeStr = getTime()
            hour =  timeStr[0:2]
            min  =  timeStr[3:5] 
            if(hour =='20' and min >='56' ):
                # do exeorder         
                print '20:56  do exeorder    '
                self.doExeOrder()
            pass
        else:
            pass   
    ...
    #----------------------------------------------------------------------
    def getTodayOpenNumber( self ):
        orders = self.mainEngine.dbQuery( 'futureauto', 'order', {"date":getDate(),"offset":"open" } ) 
        return len(orders)  
    ...
    def doExeOrder( self  ):
        ...
        orders = self.mainEngine.dbQuery( 'futureauto', 'order', {"state":0} ) 
        ...
        for order in  orders: 
            ...
            if( order['type']  == 'cancel'):
                ...
                nCount  = self.getTodayOpenNumber()
                print "opennumber:", nCount
                if( order['offset'] =='open' and nCount>= DAY_MAX_OPEN_COUNT ):
                    print 'open count  >= DAY_MAX_OPEN_COUNT '
                    continue
                directionStr = ''
                if( order['type'] =='buy' ):
                    directionStr =  u'多'
            ...
            txt ="通知：{0} {1} {2} {3} {4}".format( order['type'] ,order['offset'], order['symbol'], order['price'] ,order['vol']  )  
            self.mainEngine.sendMsg("", txt)  
               
            try:
                flt ={ "symbol":order["symbol"], "insertdt":order["insertdt"] }
