...
class MainWindow(QtWidgets.QMainWindow):
    ...
    __symbList =[{"symbol":"ru1901", "exchange":"SHFE"},{"symbol":"rb1901", "exchange":"SHFE"} ]
    ...
    #----------------------------------------------------------------------
    def loadFutures(self):
        filename = 'subFutures.json'
        try:
            f = file( filename )
        except IOError:
            log = VtLogData()
    ...
    def loadAllFutures(self):
        ...
        for key in l2:
            contract = d[key]   
            if( contract.productClass  == u'期货' and  contract.exchange != 'CFFEX' ):
                print contract.symbol,   contract.exchange , contract.productClass
                futures.append(  {"symbol": contract.symbol, "exchange": contract.exchange }  )  
        return futures
        pass
    ...
    #----------------------------------------------------------------------
    def subFutures(self):
        furueList = self.loadFutures()
        print type(furueList)
        print furueList
        for symbol in furueList:
            print symbol
            req = VtSubscribeReq()
            req.symbol = symbol[u"symbol"] #'ru1901'
            req.exchange = symbol[u"exchange"] #'SHFE'            
...
class AboutWidget(QtWidgets.QDialog):
    ...
    def initUi(self):
        """"""
        self.setWindowTitle(vtText.ABOUT + 'MyTrader')
